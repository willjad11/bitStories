...
const Feed = (props) => {
    ...
    useEffect(() => {
        ...
        user && axios.get(`http://localhost:8000/api/posts/${user.id}`)
            .then(res => {
                setPosts(res.data)
            })
            .catch(err => {
    ...
    useEffect(() => {
        axios.defaults.withCredentials = true;
        user && posts && axios.get(`http://localhost:8000/api/followedusers/${user.id}`)
            .then(res => {
                ...
                <div className={styles.navRight}>
                    ...
                    {loaded && 
                        <div className={styles.navRightButtons}>
                            <button className={styles.logoutButton} onClick={() => history.push(`/profile/${user.id}`)}>My Profile</button>
            ...
            <div className={styles.feedCont}>
                ...
                {loaded 
                    && posts.length == 0
                        ...
                        : posts.map((post, i) => 
                            <div className={styles.feedPost} key={i}>
                                <div>
                                    {followedUsers && <img className={styles.feedAvatar} onClick={() => history.push(`/profile/${post.authorID}`)} src={
                                ...
                                <div className={styles.feedProfileName}>
                                    {followedUsers && <p>{followedUsers[`${post.authorID}`].firstName} {followedUsers[`${post.authorID}`].lastName}</p>}
                                    <i><span className={styles.feedPostDate}>{post.createdAt.slice(0, 10)}</span></i>
                                </div>
